...
//
// Load OBJ file as set of triangles
//
void LoadOBJFile(const char* file, float M[], Material mat)
{
   int  Nv,Nn,Nt;  //  Number of vertex, normal and textures
   int  Mv,Mn,Mt;  //  Maximum vertex, normal and textures
   float* V;       //  Array of vertexes
   ...
   while ((line = readline(f)))
   {
      ...
      if (line[0]=='v' && line[1]==' '){
         ...
         V[Nv-3] = M[0] * x1 + M[4] * y1 + M[8] * z1 + M[12];
         V[Nv-2] = M[2] * x1 + M[5] * y1 + M[9] * z1 + M[13];
         V[Nv-1] = M[3] * x1 + M[6] * y1 + M[10] * z1 + M[14];
      ...
      else if (line[0]=='f')
      {
         ...
         while ((str = getword(&line)))
         {
            ...
            Kn[i] = -1;
            //  Try Vertex/Texture/Normal triplet
            if (sscanf(str,"%d/%d/%d",&Kv[i],&Kt[i],&Kn[i])==3)
            {
               if (Kv[i]<0 || Kv[i]>Nv/3) Fatal("Vertex %d out of range 1-%d\n",Kv,Nv/3);
               if (Kn[i]<0 || Kn[i]>Nn/3) Fatal("Normal %d out of range 1-%d\n",Kn,Nn/3);
               if (Kt[i]<0 || Kt[i]>Nt/2) Fatal("Texture %d out of range 1-%d\n",Kt,Nt/2);
            }
            //  Try Vertex//Normal pairs
            else if (sscanf(str,"%d//%d",&Kv[i],&Kn[i])==2)
            {
               if (Kv[i]<0 || Kv[i]>Nv/3) Fatal("Vertex %d out of range 1-%d\n",Kv,Nv/3);
               if (Kn[i]<0 || Kn[i]>Nn/3 ) Fatal("Normal %d out of range 1-%d\n",Kn,Nn/3);
         ...
         if(v1>=0 && v2>=0 && v3>=0){ 
            objects.push_back(RayTriangleInit((cl_float3){V[v1],V[v1+1],V[v1+2]} ,
                                             (cl_float3){V[v2],V[v2+1],V[v2+2]}, 
                                             (cl_float3){V[v3],V[v3+1],V[v3+2]} , 
                                             mat));
         }
